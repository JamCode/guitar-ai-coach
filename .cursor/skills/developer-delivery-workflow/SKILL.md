...
## Flutter 本地数据与冷启动（必须遵守）

凡触及 **SharedPreferences、应用沙箱 JSON/文件、首屏/多 Tab 挂载顺序、`main` 内初始化**，须同时遵守规则 **`flutter-mobile-robustness.mdc`**（`globs: flutter_app/**/*.dart`，编辑 Dart 时自动适用）：持久化安全读取、损坏降级、练习/重 IO Tab 懒挂载、异步与 `unawaited` 边界。
