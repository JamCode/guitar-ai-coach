...
- Native iOS：`swift_ios_host/` Xcode 宿主工程。
- 部署：`deploy/`，ECS、Nginx、systemd、PaddleOCR / ONNX 辅助服务配置。

以后接手时优先判断需求属于 Web、Backend 还是 iOS。当前主要产品形态偏 iOS；Swift 相关功能统一以 `swift_ios_host/` 为主线。
...
- `Sources/Core/`：主题、音频、调音/识别底层、公共工具。
- `Sources/Features/`：基础功能模块，包括 Tuner、Fretboard、Chords、ChordChart、Ear、Metronome、Profile、Practice 等。
...
- `Sources/StemSeparation/`：音轨分离相关模型、服务、存储与 UI。
...
- Ear：`backend/code/ear_db.py`、Swift `swift_ios_host/Sources/Features/Ear`。
...
- 练习模块基础能力：`swift_ios_host/Sources/Features/Practice`。
